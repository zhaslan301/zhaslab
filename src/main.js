...
function renderHomeDashboard() {
  ...
  const recentHtml =
    recentIngredients.length === 0
      ? renderEmptyDataMessage({ compact: true })
      : recentIngredients
          .map((ingredient) => {
            const costPerMl = formatCostPerMl(ingredient.bottlePrice, ingredient.bottleVolume);
            return `
...
function renderEmptyDataMessage({ compact = false, withActions = false } = {}) {
  const actionsHtml = withActions
    ? `
      ...
      <div class="empty-state__actions">
        ...
        <button type="button" class="btn btn--secondary empty-state__cta" id="empty-import-ingredient-btn">
          ${ICON_IMPORT}
          Импорт Excel
        </button>
      </div>
  ...
  return `
    <div class="empty-state${compact ? ' empty-state--compact' : ''}">
      <p class="empty-state__title">Пока нет данных</p>
      ${compact ? '' : EMPTY_STATE_ARROW}
      <p class="empty-state__hint">Добавьте первый ингредиент или импортируйте Excel.</p>
      ${actionsHtml}
    </div>
  `;
}
...
function renderIngredientsEmptyState() {
  return renderEmptyDataMessage({ withActions: true });
}
...
function focusIngredientImport() {
  document.querySelector('[data-tab="ingredients"]')?.click();
  const formCard = document.getElementById('ingredient-form')?.closest('.card');
  formCard?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  document.getElementById('ingredient-import')?.click();
}

function bindIngredientsEmptyState() {
  const addBtn = document.getElementById('empty-add-ingredient-btn');
  const importBtn = document.getElementById('empty-import-ingredient-btn');

  if (addBtn && !addBtn.dataset.bound) {
    addBtn.dataset.bound = 'true';
    addBtn.addEventListener('click', focusIngredientForm);
  }

  if (importBtn && !importBtn.dataset.bound) {
    importBtn.dataset.bound = 'true';
    importBtn.addEventListener('click', focusIngredientImport);
  }
}
